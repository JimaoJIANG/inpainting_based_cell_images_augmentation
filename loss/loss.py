...
def loss_fun_gen(pred, gt, mask):
    ...
    ones_mask = torch.ones_like(pred_masked).to(pred.device)
    zeros_mask = torch.zeros_like(pred_masked).to(pred.device)
