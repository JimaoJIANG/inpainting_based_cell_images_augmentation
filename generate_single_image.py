...
def main_worker(args):
    ...
    while count < 50:
        ...
        if filename[0].split('.')[0] == args.file_name:
            count += 1
            print(f'{count} / 5')
            ...
            for i in range(image.shape[0]):
                ...
                masked_seg_tmp = (255 * masked_seg.detach().cpu().numpy()).astype(np.uint8)
                comp_imgs_tmp = (
                ...
                Image.fromarray(comp_imgs_tmp).save(
                    os.path.join(args.outputs, 'comp_results', f'{filename_tmp}_{count}.jpg'))
                Image.fromarray(image_tmp).save(
                    os.path.join(args.outputs, 'gts', f'{filename_tmp}_{count}.jpg'))
                Image.fromarray(seg_tmp).save(
                    os.path.join(args.outputs, 'segs', f'{filename_tmp}_{count}.jpg'))
                Image.fromarray(masked_seg_tmp).save(
                    os.path.join(args.outputs, 'masked_segs', f'{filename_tmp}_{count}.jpg'))
