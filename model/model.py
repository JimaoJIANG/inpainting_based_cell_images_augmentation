...
class EResBlock(nn.Module):
    def __init__(self, inc, outc, use_SN=True, wide=True,
        ...
        if self.use_SN:
            self.conv1, self.conv2, self.conv_sc = spectral_norm(self.conv1), spectral_norm(self.conv2), spectral_norm(
                self.conv_sc)
...
def D_unet_arch(ch=64, attention='64'):
    ...
    arch[256] = {'in_channels': [4] + [ch * item for item in [1, 2, 4, 8, 8, 16, 8 * 2, 8 * 2, 4 * 2, 2 * 2, 1 * 2, 1]],
                 ...
                 'attention': {2 ** i: 2 ** i in [int(item) for item in attention.split('_')]
                               for i in range(2, 13)}}
    arch[512] = {'in_channels': [4] + [ch * item for item in [1, 2, 4, 8, 8, 16, 8 * 2, 8 * 2, 4 * 2, 2 * 2, 1 * 2, 1]],
                 'out_channels': [item * ch for item in [1, 2, 4, 8, 8, 16, 8, 8, 4, 2, 1, 1]],
                 'downsample': [True] * 6 + [False] * 6,
                 'upsample': [False] * 6 + [True] * 6,
                 'resolution': [256, 128, 64, 32, 16, 8, 16, 32, 64, 128, 256, 512],
                 'attention': {2 ** (i + 1): 2 ** i in [int(item) for item in attention.split('_')]
                               for i in range(2, 13)}
                 }

    arch[448] = {'in_channels': [4] + [ch * item for item in [1, 2, 4, 8, 8, 16, 8 * 2, 8 * 2, 4 * 2, 2 * 2, 1 * 2, 1]],
                 'out_channels': [item * ch for item in [1, 2, 4, 8, 8, 16, 8, 8, 4, 2, 1, 1]],
                 'downsample': [True] * 6 + [False] * 6,
                 'upsample': [False] * 6 + [True] * 6,
                 'resolution': [224, 112, 56, 28, 14, 7, 14, 28, 56, 112, 224, 448],
                 'attention': {7 * (2 ** (i - 2)): 2 ** i in [int(item) for item in attention.split('_')]
                               for i in range(2, 13)}
                 }

    arch[960] = {'in_channels': [4] + [ch * item for item in [1, 2, 4, 8, 8, 16, 8 * 2, 8 * 2, 4 * 2, 2 * 2, 1 * 2, 1]],
                 'out_channels': [item * ch for item in [1, 2, 4, 8, 8, 16, 8, 8, 4, 2, 1, 1]],
                 'downsample': [True] * 6 + [False] * 6,
                 'upsample': [False] * 6 + [True] * 6,
                 'resolution': [480, 240, 120, 60, 30, 15, 30, 60, 120, 240, 480, 960],
                 'attention': {15 * (2 ** (i - 2)): 2 ** i in [int(item) for item in attention.split('_')]
                               for i in range(2, 13)}
                 }
...
class UnetDiscriminator(BaseNetwork):
    ...
    def __init__(self, args, D_ch=64,
        ...
        self.ch = D_ch
        # Resolution
        self.resolution = args.crop_size
        # Attention?
